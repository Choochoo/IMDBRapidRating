...
const TmdbDetails = Object.freeze({ id: 101, poster_path: "/poster.jpg", overview: "TMDB synopsis.", original_language: "en", production_countries: [{ iso_3166_1: Country }], credits: { cast: [{ name: "Actor One" }] }, videos: { results: [{ site: "YouTube", key: "official_trailer", type: "Trailer", official: true }] } });
...
async function VerifyFreshDatabaseMetadata() {
  ...
  const result = await GetTitleMetadata(titleId, BuildOptions(store, RejectUnexpectedFetch));
...
async function VerifyFallbackUpgrade() {
  ...
  assert.equal(result.payload.synopsis, "TMDB synopsis.");
  assert.equal(state.urls.filter((url) => url.includes("api.themoviedb.org")).length, 1);
}
...
async function VerifyMetadataSingleFlight() {
  ...
  assert.deepEqual(results.map((result) => result.payload.synopsis), ["TMDB synopsis.", "TMDB synopsis."]);
}
...
function BuildOriginMetadata(titleId) {
  return { titleId, mediaType: MovieMediaType, status: "matched", tmdbId: 101, originCountries: [Country], originalLanguage: "en", checkedAt: OldTimestamp, metadataCheckedAt: "", source: "", sourcePayload: {}, actors: [], trailerUrl: "", streamingByCountry: {} };
}

function BuildStoredMetadata(titleId, overrides = {}) {
  return { ...BuildOriginMetadata(titleId), posterUrl: "https://image.tmdb.org/t/p/w342/poster.jpg", synopsis: "Stored locally.", actors: ["Actor One"], source: TmdbSource, sourcePayload: TmdbDetails, metadataCheckedAt: new Date().toISOString(), ...overrides };
}

function BuildAvailability() {
  return { country: Country, fetchedAt: new Date().toISOString(), watchUrl: "https://www.themoviedb.org/movie/101/watch?locale=US", providers: [{ type: "subscription", id: 8, name: "Netflix", logoPath: "/netflix.jpg", displayPriority: 1 }] };
}
...
function CreateMetadataFetch(state, detailsSucceed = true) {
  return async (url) => {
    ...
    if (String(url).includes("imdb.com/title"))
